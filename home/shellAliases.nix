...

{
  programs.bash = {
    shellAliases = {
      ...
      rebuild = "sudo nixos-rebuild switch --flake path:${config.home.homeDirectory}/CirnOS#$(hostname)";
      update = "cd ${config.home.homeDirectory}/CirnOS && sudo nix flake update && sudo nixos-rebuild switch --flake path:.#$(hostname)";

...
Item {
    ...
    property string startThresholdFile: `${pluginApi?.pluginSettings?.batteryDevice || service.batteries[0]}/charge_control_start_threshold`
    ...
    function setThreshold(value) {
        ...
        const start = Math.max(0, v - 5);
        Logger.i("BatteryThreshold", "Restored charge threshold to " + v + "%");

        thresholdWriter.pendingThreshold = v;
        thresholdWriter.command = ["sh", "-c", `set -e; if [ -e ${startThresholdFile} ]; then echo ${start} > ${startThresholdFile}; fi; echo ${v} > ${thresholdFile}`];
    ...
    Process {
        ...
        command: ["sh", "-c", `test -w ${root.thresholdFile} && { [ ! -e ${root.startThresholdFile} ] || test -w ${root.startThresholdFile}; } && echo 1 || echo 0`]
        running: false
